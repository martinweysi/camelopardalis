...
"""
================================================================================
Campaign A - The Complete Model
Version: 15.7 (Physics Completion: Three-Term Nonlinear Potential)
Author: Martin Weysi
Date: October 13, 2025

Description:
This script represents the culmination of our entire diagnostic process. The
failure of v15.6 (runaway collapse) was a critical piece of data, proving
that an attractive eta term alone is insufficient and leads to blow-up.

This version introduces the final piece of the physical puzzle: a higher-order
*repulsive* term (+xi*|psi|^8). This completes the phenomenological potential,
giving it the necessary structure to support a stable, non-collapsing droplet:
1. Cubic repulsion (lambda) at low densities.
2. Sextic attraction (eta) at intermediate densities to form a potential well.
3. Octic repulsion (xi) at very high densities to prevent collapse.

This is the most physically complete and sophisticated version of our model. Its
success or failure is the definitive result of Campaign A.

Key Upgrades:
1.  **Complete Phenomenological Potential:** The V_deriv now includes the crucial
    eighth-order repulsive term, providing the final stabilization mechanism.
2.  **The Definitive Test:** This simulation directly tests the hypothesis that
    a potential with a local minimum (created by competing nonlinearities) is
    necessary and sufficient for stable droplet formation in this regime.
================================================================================
"""
...
class CompleteModelSimulator:
    """
    An object-oriented framework for the complete three-term nonlinear potential model.
    """
    ...
    def __init__(self, phys_params, sim_params):
        ...
        print("--- CompleteModelSimulator v15.7 (Three-Term Potential) Initialized ---")
    ...
    def run_real_time_simulation(self):
        print("================================================================")
        print("=== Campaign A, v15.7: The Definitive Test                   ===")
        print("================================================================")
        ...
        for i in range(1, total_steps + 1):
            ...
            # Includes all three competing nonlinear terms
            V_deriv = (-self.phys_params['mu_sq'] + 
                       2 * self.phys_params['lambda'] * psi_sq + 
                       3 * self.phys_params['eta'] * psi_sq**2 +
                       4 * self.phys_params['xi'] * psi_sq**3)
    ...
    def analyze_and_plot(self):
        print("\n--- Analysis and Plotting of the Final State ---")
...
if __name__ == "__main__":
    phys_params = {
        'mu_sq': 1.0, 
        'lambda': 0.6,    # Strong Repulsion
        'eta': -0.1,      # <<< STRONG ATTRACTION to form the well
        'xi': 0.01,       # <<< THE FINAL BRAKE: positive 8th-order repulsion
    ...
    sim_params = {
        'N': 256, 'L': 40.0,
        'dt_real': 0.005,
        'total_steps': 200000, # Give it ample time to settle
        'log_interval': 500
    }

    simulator = CompleteModelSimulator(phys_params, sim_params)

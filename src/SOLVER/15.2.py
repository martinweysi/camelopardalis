...
experimental paper (Ballarini et al., PRL 2019, building on the foundations of
Kasprzak et al., Nature 443, 409).

This is no longer just a simulation; it is a numerical experiment designed to
test the model's viability in a physically relevant, and much harsher, regime.
The outcome, whether it forms a droplet or results in a different state, is a
meaningful scientific prediction.

Key Upgrades:
1.  **Experimentally Calibrated Physics:** The parameters `gamma_c` and `lambda`
    have been updated to reflect the real-world values derived from a high-Q
    GaAs microcavity experiment. The interaction is much stronger, and the loss
    is much weaker.
2.  **Reinforced Numerical Parameters:** `dt_real` is reduced for stability against
    stronger interactions, and `total_steps` is increased to allow the system
    with lower loss more time to relax into a potential steady state.
================================================================================
"""
...
class PolaritonAnalyzer:
    ...
    def __init__(self, phys_params, sim_params):
        ...
        print("--- Parameters calibrated based on high-Q GaAs system analysis ---")
...
if __name__ == "__main__":
    # --- EXPERIMENTALLY CALIBRATED PARAMETERS (v15.2) ---
    # Based on analysis of high-Q GaAs systems. This is a harsh, physically
    # relevant regime: strong interactions, low loss.
    phys_params = {
        'mu_sq': 1.0,      # A free parameter to drive condensation.
        'lambda': 0.6,     # <<< REALISTIC (STRONG) INTERACTION
        'eta': 0.1,        # Saturation must be stronger to tame the strong lambda.
        'kappa': 1.0,      
        'gamma_c': 0.01,   # <<< REALISTIC (LOW) LOSS
        'gamma_r': 2.0,    # Reservoir loss is typically much faster than condensate loss.
        'P0': 1.5,         # A moderate pump. May need adjustment.
        'w0': 5.0,         
        'D': 0.05, 
        'omega': -2.0       # Chemical potential needs to be adjusted for stronger interactions.
    }
    
    sim_params = {
        'N': 256, 'L': 40.0,
        'dt_real': 0.005,        # <<< SMALLER timestep for stability
        'total_steps': 150000,   # <<< LONGER simulation time
        'log_interval': 500
    }

...
"""
================================================================================
The Non-Local Model: The Final Attempt for a Single Droplet
Version: 16.2 (Surgical Strike Experiment)
Author: Martin Weysi
Date: October 13, 2025

Description:
This script is the definitive and final test of Campaign A. The failure of v16.1
to produce a single droplet was a critical scientific result: it proved that
modest changes to the pump are insufficient to overcome the system's intrinsic
tendency towards a turbulent, multi-peaked state.

This version tests a final, more extreme hypothesis: a single droplet can only
be formed if the pump itself is engineered to be a "single-mode seed," with a
width comparable to the expected droplet size and a power just above the absolute
condensation threshold. This is no longer a gentle nudge; it is a surgical strike.

This is the last attempt to generate a clean Figure 2 before concluding that
such a state is impossible in this parameter regime.

Key Upgrades:
1.  **Surgical Pumping:** The pump width (w0) and power (P0) have been
    drastically reduced to create a highly localized, near-threshold excitation.
2.  **The Definitive Test:** The goal is to provide a final, unambiguous yes/no
    answer to the question: "Can this non-local model support a clean, single,
    stable droplet under *any* reasonable conditions?"
================================================================================
"""
...
class NonLocalSimulator:
    ...
    def __init__(self, phys_params, sim_params):
        ...
        print("--- NonLocalSimulator v16.2 (Surgical Strike) Initialized ---")
    ...
    def run_real_time_simulation(self):
        print("================================================================")
        print("=== v16.2: The Final Attempt for a Single Droplet            ===")
        print("================================================================")
...
if __name__ == "__main__":
    # Parameters for the surgical strike
    phys_params = {
        'mu_sq': 1.0, 
        'lambda': 1.0,
        'kappa': 1.0, 'gamma_c': 0.01, 'gamma_r': 2.0,
        'P0': 1.0,         # <<< SURGICAL STRIKE: Very low pump power
        'w0': 2.5,         # <<< SURGICAL STRIKE: Very narrow pump
        'omega': -2.0,
        'g_nonlocal': 1.5,
        'r_nonlocal': 4.0,
    }
    ...
    sim_params = {
        'N': 256, 'L': 40.0,
        'dt_real': 0.01,
        'total_steps': 150000, # Give it enough time to stabilize
        'log_interval': 500
    }

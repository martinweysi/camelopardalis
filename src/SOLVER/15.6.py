...
"""
================================================================================
Campaign A - The Attraction Hypothesis
Version: 15.6 (Physics Revolution: Attractive Higher-Order Term)
Author: Martin Weysi
Date: October 13, 2025

Description:
This script embodies a radical but physically motivated shift in strategy. All
previous attempts (v15.2-v15.5) failed to stabilize the condensate in the
realistic parameter regime. This null result is interpreted as a failure of
the model's core assumption: that all nonlinearities are repulsive.

This version tests a new, profound hypothesis: that at high densities, competing
physical effects (like screening or biexciton formation) lead to an effective
*attractive* higher-order interaction. This is modeled by changing the sign of
the sextic coefficient `eta` to be negative.

This transforms the potential from a simple monotonic function to one with a
local minimum, providing a natural self-trapping mechanism that can fight the
strong cubic repulsion. This is our most sophisticated attempt to explain
stability.

Key Upgrades:
1.  **Fundamental Physics Change:** The sign of `eta` is flipped from positive
    (repulsive saturation) to negative (attractive saturation), which
    fundamentally changes the nature of the potential V(|psi|^2).
2.  **The Final Hypothesis Test:** This simulation is the ultimate test for
    Campaign A. If a stable droplet forms now, it provides a powerful, non-trivial
    prediction about the nature of many-body interactions in this system.
================================================================================
"""
...
class AttractionHypothesisSimulator:
    """
    An object-oriented framework for testing the effect of an attractive
    ...
    def __init__(self, phys_params, sim_params):
        ...
        print("--- AttractionHypothesisSimulator v15.6 (Attractive Eta) Initialized ---")
    ...
    def run_real_time_simulation(self):
        print("================================================================")
        print("=== Campaign A, v15.6: Testing the Attraction Hypothesis     ===")
        print("================================================================")
        ...
        for i in range(1, total_steps + 1):
            ...
            psi_sq = np.abs(psi)**2
            # The potential V'(|psi|^2) now has a competing attractive term
...
if __name__ == "__main__":
    phys_params = {
        ...
        'eta': -0.05,       # <<< THE REVOLUTIONARY HYPOTHESIS: ATTRACTIVE ETA
        'kappa': 1.0, 'gamma_c': 0.01, 'gamma_r': 2.0,
        'P0': 1.8,         # A slightly stronger pump to reach the new potential minimum
    ...
    simulator = AttractionHypothesisSimulator(phys_params, sim_params)
